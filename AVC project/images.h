...
int set_pixel(ImagePPM &image, int row, int col, unsigned char red,
...
int draw_background(ImagePPM &image) {
  ...
  for (int col = 0; col < image.width; col++)
    for (int row = 0; row < image.height; row++) {
      set_pixel(image, row, col, 180 + rand() % rr, 180 + rand() % rr,
                180 + rand() % rr);
    }
  return 0;
}
...
int draw_circle(ImagePPM &image, Point2Di c, int R, char r, char g, char b) {
  for (int col = c.x - R; col < c.x + R; col++)
    for (int row = c.y - R; row < c.y + R; row++) {
      int dx = col - c.x;
      int dy = row - c.y;
      if ((row > 0) && (row < image.height) && (col > 0) &&
          (col < image.width) && (dx * dx + dy * dy < R * R))
        set_pixel(image, row, col, r, g, b);
    }
  return 0;
}

int draw_square(ImagePPM &image, int xc, int yc, int W, char r, char g,
                char b) {
  int xtop = xc - W / 2;
  int ytop = yc - W / 2;
  int xbot = xc + W / 2;
...
int draw_line(ImagePPM &image, Point2Di p1, Point2Di p2) {
  //	std::cout<<"draw line:"<<p1.x<<" "<<p1.y<<" "<<p2.x<<"
  //"<<p2.y<<std::endl;
  float x1 = p1.x;
  float y1 = p1.y;
  float x2 = p2.x;
...
void pick_camera(Robot robot) {
  ...
  robot.camera_current =
      robot.camera_templ.rotate_move(robot.cent, robot.pos, -robot.azimuth);

  for (int cam_row = 0; cam_row < robot.fovLength; cam_row++) {
    double u = (double)cam_row / (double)robot.fovLength;
    double xa = robot.camera_current.points[1].x +
                u * (robot.camera_current.points[0].x -
                     robot.camera_current.points[1].x);
    double ya = robot.camera_current.points[1].y +
                u * (robot.camera_current.points[0].y -
                     robot.camera_current.points[1].y);

    double xb = robot.camera_current.points[2].x +
                u * (robot.camera_current.points[3].x -
                     robot.camera_current.points[2].x);
    double yb = robot.camera_current.points[2].y +
                u * (robot.camera_current.points[3].y -
                     robot.camera_current.points[2].y);
...
void draw_robot(Robot &robot) {
  robot.body_current =
      robot.body_templ.rotate_move(robot.cent, robot.pos, -robot.azimuth);
  robot.camera_current =
      robot.camera_templ.rotate_move(robot.cent, robot.pos, -robot.azimuth);
...
void init(int rx0, int ry0, double az) {
  ...
  if (init_images(2200, 1650, robot) == 0) {
    ...
    std::cout << " Fail to reserve image memory" << std::endl;
  }
}
